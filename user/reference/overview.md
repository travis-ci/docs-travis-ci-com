...
| `.travis.yml`        | `dist: bionic` or `dist: xenial` or `dist: trusty` or `dist: precise`                                                                                         | `os: osx`                     | `os: windows`                      | `os: linux arch: arm64-graviton2 dist: focal` or `os: linux arch: arm64-graviton2 dist: bionic` or `os: linux arch: arm64-graviton2 dist: xenial` or `os: linux arch: arm64 dist: bionic` or `os: linux arch: arm64 dist: xenial`or `os: linux arch: ppc64le dist: bionic` or `os: linux arch: ppc64le dist: xenial` or `os: linux arch: s390x dist: bionic` or `os: linux arch: s390x dist: xenial`                              |
