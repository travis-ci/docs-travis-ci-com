...
By default, Travis CI Enterprise verifies SSL for all traffic between its various components. SSL verification helps ensure that traffic is delivered between components securely. It is **strongly recommended** that you configure your installation with a valid certificate from a trusted authority.
...
> If you decide at a later time to upload your own custom certificate then this can be done at any time via the [admin dashboard](#how-to-add-or-update-certificates-after-installation).

#### Trial Users

If you are using a trial license to evaluate Travis CI Enterprise then going through the effort of obtaining a trusted certificate might not make sense at this time. For this scenario we offer the ability to use self-signed certificates that are generated by Travis CI Enterprise itself.
...
#### Without a root certificate from a trusted CA

If your internal CA is not using a root certificate provided by a trusted CA then any certificates produced are considered to be 'untrusted' by most tools and browsers, same as a self-signed certificate.

Certificates created by an internal CA can be uploaded during installation in the same way as any other certificate.
...
#### With a root certificate from a trusted CA

If your organization's internal CA does use a root certificate purchased from a trusted CA then the above will not apply. You will not need to disable SSL verification and can proceed as if you are using a certificate directly from a trusted CA.
...
You can disable SSL verification across Travis CI Enterprise via the following steps:

- Log into your dashboard at `https://<your-travis-ci-enterprise-domain>:8800`.
- Click on 'Settings' in the topmost menu.
- Click 'Advanced Settings' in the left menu (or scroll down).
- Check the box for 'Disable SSL cert verification'.
...
- Log into your dashboard at `https://<your-travis-ci-enterprise-domain>:8800`.
- Click the gear icon on the rightmost side of the top menu and select 'Console Settings' from the dropdown.
- Click 'TLS Key & Cert' in the left menu (or scroll down)
- Select the appropriate option depending on the location of your certificate files
- Enter the paths or select the files to upload
- Click 'Save' at the bottom of the page
...
- Click on 'Settings' in the topmost menu.
- Click 'Advanced Settings' in the left menu (or scroll down).
- Copy and paste your certificate chain into the text box labeled 'Custom Certificate Authority (CA) Bundle'.
- Click 'Save' at the bottom of the page.
