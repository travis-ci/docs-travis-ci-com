...
changing is the README for example, add `[ci skip]` or `[skip ci]` to the git commit message.

Commits that have `[ci skip]` or `[ci skip]` anywhere in the commit messages are ignored by
Travis CI. 
