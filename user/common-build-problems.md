...
3. [OS X](https://docs.docker.com/docker-for-mac/)
4. [Ubuntu Linux](https://docs.docker.com/engine/installation/linux/ubuntulinux/)
5. Select an image from [Quay.io](https://quay.io/organization/travisci). If you're not using a language-specific image pick `travis-ruby`. Open a terminal and start an interactive Docker session using the image URL:

   ```bash
   docker run -it quay.io/travisci/travis-ruby /bin/bash
   ```

6. Switch to the `travis` user:

   ```bash
   su - travis
   ```

7. Clone your git repository into the `/` folder of the image.
8. Manually install any dependencies.
